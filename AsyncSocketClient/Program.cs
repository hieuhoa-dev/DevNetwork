...
namespace AsyncSocketClient
{
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Console.Write("Enter Part Number (0 - 65535):");
            string strPortInput = Console.ReadLine();

            if (!client.SetServerIPAddress(strIPAddress) || !client.SetPortNumber(strPortInput))
            {
                Console.WriteLine(string.Format("IP Address or port number invalid- {0} {1} Press a key to exit", strIPAddress, strPortInput));
                Console.ReadKey();
                return;
            }
